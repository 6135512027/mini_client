    ...
    async (req, res) => {
        try {
            ...
            if (db.checkExistingUser(username) !== db.NOT_FOUND)
                return res.json({ message: "ข้อมูลซ้ำหรือไม่ถูกต้อง" })

            let id = (users.users.length) ? users.users[users.users.length - 1].id + 1 : 1
            hash = await bcrypt.hash(password, SALT_ROUND)
            users.users.push({ id, username, password: hash, email })
            res.status(200).json({ message: "การลงทะเบียนเสร็จสมบูรณ์" })
        } catch {
            res.status(422).json({ message: "ไม่สามารถลงทะเบียนได้" })
        }
    })
...
let students = {
    list: [{ "id": 1, "name": "Nattapat","surname": "sinra", "major": "CoE", "GPA" : 2.84 }]
}
